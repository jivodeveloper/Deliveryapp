...
class DeliveryDataState extends State<DeliveryData> {
  ...
  List<bool> cb_value = [];
  List<int> item_id_list = [];

  bool payement_status = false;
  int item=0;
  bool? check1 = false;
  int userid=0;
    ...
    return WillPopScope(
      ...
      child:Scaffold(
        ...
        appBar: AppBar(
          ...
          backgroundColor: Color(0xff1b3664),
  ...
  Future<void> showitemlist(BuildContext contextt,list,orderId,item) async {
    ...
    setState((){
      cb_value = List<bool>.filled(list.length, false);
    });
                      ...
                      children: [
                                        ...
                                        children: [
                                          ...
                                          Expanded(
                                              ...
                                              child: CheckboxListTile(
                                                  value: cb_value[index],
                                                  onChanged: (newValue) {

                                                    if(newValue == true){

                                                      item += list[index].itemTotalAmount!;
                                                      print("item list ${list[index].odrItemId}");

                                                      setState((){
                                                        item_id_list.add(list[index].odrItemId);
                                                      });

                                                    }else{

                                                      item =0;
                                                     // print("length1 ${item_id_list.length} $index");

                                                      setState((){
                                                        item_id_list.remove(list[index].odrItemId);
                                                      });

                                                      //print("length2 ${item_id_list.length}");

                                                    }

                                                    setState((){
                                                      cb_value[index] = newValue!;
                        ...
                        GestureDetector(
                            ...
                            onTap: () {
                              ...
                              print("item length ${item_id_list.length}");
                              ...
                              if(item_id_list.length!=0){
                                ...
                                Fluttertoast.showToast(msg: "Select item");
                              }
                              ...
                              child:  GestureDetector(
                                  onTap: () {
                                    ...
                                    if(payement_status==true){
                                      ...
                                      Fluttertoast.showToast(msg: "Please update payment details");
                                ...
                                child:GestureDetector(
                                  ...
                                  onTap: () {
                                   ...
                                   if(item_id_list.length !=0) {
                                    ...
                                    for(int i=0;i<selec_itemid.length;i++){
                                      print("${selec_itemid[i]}");
                                    }
    ...
    try{
      ...
      for(int i=0;i<orderList.length;i++){
        ...
        for(int j=0;j<orderList[i].itemList!.length;j++) {
          print("itemlist ${orderList[i].itemList![j].odrItemId}");
        }
  ...
  print("$status ${item_id_list.toString()} $orderid");

  var response = await http.post(
    Uri.parse('${Common.IP_URL}UpdateDeliveryStatus?newStatus=$status&itemidJSON=${item_id_list.toString()}&OID=$orderid'),
  ...
  if (response_data['Message'] == "Delivery status updated successfully.") {
    ...
    Fluttertoast.showToast(msg: "Delivery Record updated succesfully",
        toastLength: Toast.LENGTH_SHORT,
        gravity: ToastGravity.BOTTOM,
        timeInSecForIosWeb: 1,
        backgroundColor: Colors.black,
  ...
  /*inse  rt payment details*/
  Future updatepaymentdata(String status,int orderid) async {

   // print("${reference_id.text} ${t_amt.text} $payment $orderid");
